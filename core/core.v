...
pub fn (mut c Container) check_circular_dependencies() ! {
	cycle := c.find_dependency_cycle()
	if cycle.len > 0 {
		path := cycle.join(' → ')
		return error('circular dependency detected: ${path} / 检测到循环依赖: ${path}')
	}
}

// find_dependency_cycle returns the first dependency cycle in the bean graph as
// an ordered path (e.g. ['A','B','C','A']), or an empty list if the graph is
// acyclic. Edges are built from both @[autowired] field dependencies and
// @[depends_on] explicit ordering. Uses DFS with a recursion stack so the exact
// cycle path is reported (unlike Kahn's algorithm, which only proves existence).
pub fn (mut c Container) find_dependency_cycle() []string {
	c.mu.rlock()
	defer { c.mu.runlock() }

	// Build adjacency list: node → the beans it directly depends on.
	mut adj := map[string][]string{}
	for name, def in c.definitions {
		mut deps := []string{}
		for dep in def.dependencies {
			if dep.type_name in c.definitions && dep.type_name !in deps {
				deps << dep.type_name
			}
		}
		for dep_name in def.depends_on {
			if dep_name in c.definitions && dep_name !in deps {
				deps << dep_name
			}
		}
		adj[name] = deps
	...
	// DFS over every node; 0=unvisited, 1=in-progress(on stack), 2=done.
	mut color := map[string]int{}
	mut stack := []string{}
	for name, _ in c.definitions {
		if color[name] == 0 {
			cycle := dfs_find_cycle(name, adj, mut color, mut stack)
			if cycle.len > 0 {
				return cycle
			}
		}
...
// dfs_find_cycle is the recursive DFS helper for find_dependency_cycle.
// On encountering a back-edge to a node currently on the recursion stack it
// reconstructs and returns the cycle path; otherwise returns an empty list.
fn dfs_find_cycle(node string, adj map[string][]string, mut color map[string]int, mut stack []string) []string {
	color[node] = 1 // in-progress
	stack << node
	for next in adj[node] {
		if color[next] == 1 {
			// Back-edge → reconstruct the cycle from `next`'s stack position.
			mut cycle := []string{}
			mut started := false
			for s in stack {
				if s == next {
			...
			cycle << next // close the loop (… → next)
			return cycle
		}
		if color[next] == 0 {
			sub := dfs_find_cycle(next, adj, mut color, mut stack)
			if sub.len > 0 {
				return sub
			}
		}
