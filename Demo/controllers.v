...
pub fn (mut app App) get_users(mut ctx Context) veb.Result {
	...
	// 转换为 UserResource 集合
	mut user_resources := []resources.UserResource{}
	for u in users {
		user_resources << resources.new_user_resource(&u)
	}

	// 使用 web.page 构建分页响应（含 pagination 元数据）
	page_result := web.page(json.encode(user_resources), page, page_size, total)
...
pub fn (mut app App) get_posts(mut ctx Context) veb.Result {
	...
	// 转换为 PostResource 集合
	mut post_resources := []resources.PostResource{}
	for p in posts {
		post_resources << resources.new_post_resource(&p)
	}

	// 使用 web.page 构建分页响应（含 pagination 元数据）
	page_result := web.page(json.encode(post_resources), page, page_size, total)
	return ctx.send_page_result(page_result)
}
...
pub fn (mut app App) get_categories(mut ctx Context) veb.Result {
	...
	mut category_resources := []resources.CategoryResource{}
	for c in categories {
		category_resources << resources.new_category_resource(&c)
	}

	// 使用 web.page 构建分页响应
	page_result := web.page(json.encode(category_resources), 1, category_resources.len, category_resources.len)
	return ctx.send_page_result(page_result)
}
...
pub fn (mut app App) get_tags(mut ctx Context) veb.Result {
	...
	// 转换为 TagResource 集合
	mut tag_resources := []resources.TagResource{}
	for t in tags {
		tag_resources << resources.new_tag_resource(&t)
	}

	// 使用 web.page 构建分页响应
	page_result := web.page(json.encode(tag_resources), 1, tag_resources.len, tag_resources.len)
	return ctx.send_page_result(page_result)
}
