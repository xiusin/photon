...
fn test_build_cache_key_simple() {
	attr := CacheableAttribute{cache_name: 'users', key_pattern: ''}
...
fn test_build_cache_key_with_pattern() {
	attr := CacheableAttribute{cache_name: 'users', key_pattern: 'user:{0}'}
	key := build_cache_key(attr, 'find_by_id', '123')
	assert key == 'users::user:123'
}

fn test_build_cache_key_with_multiple_pattern_args() {
	attr := CacheableAttribute{cache_name: 'users', key_pattern: 'user:{0}:role:{1}'}
...
fn test_build_evict_key_simple() {
	attr := CacheEvictAttribute{cache_name: 'users', key_pattern: ''}
	key := build_evict_key(attr, 'delete_user')
	assert key == 'users::delete_user'
}

fn test_build_evict_key_with_pattern() {
	attr := CacheEvictAttribute{cache_name: 'users', key_pattern: 'user:{0}'}
...
fn test_new_cacheable_interceptor() {
	ci := new_cacheable_interceptor(new_cache_manager())
	assert !isnil(ci.cache_manager)
}

fn test_cacheable_interceptor_get_or_compute() {
	mut ci := new_cacheable_interceptor(new_cache_manager())
	attr := CacheableAttribute{cache_name: 'default', ttl_seconds: 60}
...
fn test_cacheable_interceptor_evict() {
	...
	// Evict it
	evict_attr := CacheEvictAttribute{cache_name: 'default'}
...
fn test_cacheable_interceptor_put() {
	mut ci := new_cacheable_interceptor(new_cache_manager())
	put_attr := CachePutAttribute{cache_name: 'default', ttl_seconds: 60}
