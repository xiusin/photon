...
.PHONY: build run test verify test-core-tests test-all check clean help docs-serve doctor link-vmodules unlink-vmodules
...
## verify — 运行框架功能验证套件 (DI/注解/各模块用法, 130+ 断言)
verify:
	$(V) $(VFLAGS) run $(ROOT_PREFIX)example/verify

## test-core-tests — 运行 core/tests/ 下的黑盒测试 (依赖循环检测等)
test-core-tests:
	$(V) $(VFLAGS) test $(ROOT_PREFIX)core/tests/
